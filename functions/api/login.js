...
export async function onRequest({ request, env }) {
  ...
  try {
    ...
    if (action !== "login") return Response.json({ ok: false, msg: "非法请求" }, { headers: corsHeaders });
    const sqlRes = await env.DB.prepare(`SELECT id, role FROM admin WHERE username = ? AND password = ?`).bind(username, password).all();
