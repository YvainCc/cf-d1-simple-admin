...
export async function onRequest({ request, env }) {
  ...
  if (request.method === "POST") {
    ...
    if (!teamId || !seasonId) return Response.json({ ok: false, msg: "参数缺失" }, { headers: corsHeaders });
    ...
    `).bind(teamId).first();
    if (existReg) return Response.json({ ok: false, msg: "该战队已提交本赛季报名" }, { headers: corsHeaders });
    const starterList = await env.DB.prepare(`
      SELECT s.historical_total_kd
      FROM team_members tm
      LEFT JOIN player_stats s ON tm.player_id = s.player_id
